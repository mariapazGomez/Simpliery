...
import { FinCard, AlertaBanner, SectionHeader } from '@/components/finanzas/shared'
...
/* ── Resumen tab ─────────────────────────────── */
export function FinResumen({ setTab }: { setTab: (t: string) => void }) {
  ...
  const indicadores: { label: string; val: string; tone: Tone; desc: string }[] = [
  ...
  const toneColor: Record<Tone, string> = { ok: 'var(--ok)', warn: 'var(--warn)', danger: 'var(--danger)' }
  const toneBg: Record<Tone | 'primary', string> = { ok: 'var(--ok-tint)', warn: 'var(--warn-tint)', danger: 'var(--danger-tint)', primary: 'var(--primary-tint)' }
  const toneFg: Record<Tone | 'primary', string> = { ok: 'var(--primary-700)', warn: 'oklch(0.50 0.10 70)', danger: 'var(--danger)', primary: 'var(--primary-700)' }

  type Decision = { tone: Tone | 'primary'; icon: string; text: string; action?: { label: string; fn: () => void } }
  ...
  return (
    <div>
      ...
      <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 18 }}>
        ...
        <div className="card">
          <div className="card-head">
            <span style={{ width: 32, height: 32, borderRadius: 9, background: 'var(--primary-tint)', color: 'var(--primary-700)', display: 'grid', placeItems: 'center' }}><Icon name="balance" size={16} /></span>
            <div style={{ flex: 1 }}><div className="card-title">Balance simplificado</div><div className="card-sub">Activos vs Pasivos vs Patrimonio</div></div>
            <button className="btn btn-ghost" style={{ fontSize: 12, padding: '5px 10px' }} onClick={() => setTab('balance')}>Ver detalle</button>
          </div>
        ...
        <div className="card">
          <div className="card-head">
            <span style={{ width: 32, height: 32, borderRadius: 9, background: 'var(--terra-tint)', color: 'var(--terra-700)', display: 'grid', placeItems: 'center' }}><Icon name="calculator" size={16} /></span>
            <div style={{ flex: 1 }}><div className="card-title">Indicadores clave</div><div className="card-sub">Métricas para tomar decisiones</div></div>
          </div>
          <div style={{ padding: '10px 0' }}>
            ...
            ] as { label: string; val: string; tone: Tone; help: string }[]).map((r, i) => (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 18 }}>
        {decisiones.map((d, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: toneBg[d.tone], borderRadius: 12, border: `1px solid ${d.tone === 'primary' ? 'var(--primary)' : toneColor[d.tone]}33` }}>
...
export function FinFlujo() {
  ...
  type Movimiento = { fecha: Date; desc: string; tipo: 'Ingreso' | 'Gasto'; monto: number; estado: string }
  const movimientos: Movimiento[] = [
    ...ingMes.slice(0, 6).map((s): Movimiento => ({ fecha: s.date, desc: `Venta boleta #${s.boleta}`, tipo: 'Ingreso', monto: s.total, estado: 'completado' })),
    ...gastosMes.map((g): Movimiento => ({ fecha: g.fecha, desc: g.desc, tipo: 'Gasto', monto: -g.monto, estado: g.estado })),
  ]
    ...
    <div>
      ...
      {m.cajaProyectada < 300000 && (
        <div style={{ marginBottom: 14 }}>
          <AlertaBanner tone="danger" icon="alert">
          ...
          <div className="card-pad">
            <ColumnChart
              ...
              series={[
                ...
                { key: 'costos', label: 'Costos+Gastos', color: 'var(--terra)' },
                { key: 'saldo', label: 'Saldo', color: 'var(--info)' },
              ]}
            />
