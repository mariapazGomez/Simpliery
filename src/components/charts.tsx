...
export function LineChart({
  ...
  const W = 400
  const H = height - 28
  const pt = (v: number, i: number, len: number): [number, number] => [(i / (len - 1 || 1)) * W, H - ((v - min) / rng) * (H - 8) - 4]
  const id = 'lc' + useId().replace(/:/g, '')
    ...
    <div style={{ position: 'relative' }}>
      ...
      >
        {[0, 0.25, 0.5, 0.75, 1].map((f, i) => (
          <line key={i} x1={0} y1={H - f * (H - 8) - 4} x2={W} y2={H - f * (H - 8) - 4} stroke="var(--line)" strokeWidth="1" opacity=".5" />
        ))}
        {series.map((s, si) => {
        ...
        {idx != null && (
          <line x1={pt(0, idx, n)[0]} y1={0} x2={pt(0, idx, n)[0]} y2={H} stroke="var(--ink-3)" strokeWidth="1" strokeDasharray="3 3" />
        )}
        {xLabels &&
          xLabels.map((l, i) => (
            <text key={i} x={(i / (xLabels.length - 1 || 1)) * W} y={H + 18} textAnchor="middle" fill="var(--ink-3)" fontSize="11" fontWeight="700" fontFamily="inherit">
              {l}
            </text>
          ))}
      </svg>
      {idx != null && (
        ...
        >
          {xLabels?.[idx] && <div style={{ opacity: 0.7, fontSize: 11, marginBottom: 3 }}>{xLabels[idx]}</div>}
          {series.map((s, si) => (
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 4, flexWrap: 'wrap' }}>
        {series.map((s, i) => (
          <span key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 600, color: 'var(--ink-2)' }}>
            <span style={{ width: 16, height: 3, borderRadius: 2, background: s.color }}></span>
            {s.label}
          </span>
        ))}
      </div>
