...
// ---------- Formatos de venta: producto madre + variantes (portado de screen-formatos.jsx) ----------
 ...
 * no lo define, así que lo tratamos como opcional localmente.
 */
type ProductWithKg = Product & { kgPerUnit?: number }

/* ---------- Format Manager Modal: producto madre + variantes ---------- */
export function FormatManagerModal({ product, onClose }: { product: ProductWithKg; onClose: () => void }) {
  ...
  const unitLabel = ({ kg: 'kg', gramo: 'gramos', litro: 'litros', mililitro: 'ml', Unidad: 'unidades', caja: 'cajas', paquete: 'paquetes' } as Record<string, string>)[unit] || unit
  const unitAbbrev = ({ kg: 'kg', gramo: 'g', litro: 'L', mililitro: 'ml', Unidad: 'u.', caja: 'cajas', paquete: 'paq.' } as Record<string, string>)[unit] || unit
  const unitStep = isWeight ? '0.001' : '1'
  const [stock, setStock] = useState<number | string>(product.stock)
  const [kgPerUnit, setKgPerUnit] = useState<number | string>(product.kgPerUnit || 0)
  const [cost, setCost] = useState<number | string>(product.cost)
  ...
  const saveBase = () => {
    updateProduct(product.id, { stock: +stock || 0, cost: +cost || 0, kgPerUnit: +kgPerUnit || 0 } as Partial<Product>)
  ...
  const variantCost = (qty: number | string) => Math.round((+cost || 0) * (+qty || 0))
  const variantMargin = (price: number | string, qty: number | string) => (+price || 0) - variantCost(qty)
  const variantMarginPct = (price: number | string, qty: number | string) => ((+price || 0) > 0 ? Math.round((variantMargin(price, qty) / +price) * 100) : 0)
  const variantKg = (qty: number | string) => (+kgPerUnit > 0 ? +(+kgPerUnit * (+qty || 0)).toFixed(3) : null)
  const maxUnits = (qty: number | string) => ((+qty || 0) > 0 ? Math.floor((+stock || 0) / +qty) : 0)

  type StripItem = { l: string; v: string; c: string }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 13 }}>
            ...
            <div>
              <div style={{ fontWeight: 800, fontSize: 15 }}>Producto madre</div>
              <div style={{ fontSize: 12.5, color: 'var(--ink-3)', fontWeight: 600 }}>El stock total desde donde descontarán todos los formatos</div>
            </div>
          </div>
          <div style={{ background: 'var(--surface-3)', borderRadius: 14, padding: '16px 18px', display: 'grid', gridTemplateColumns: isWeight ? '1fr 1fr' : '1fr 1fr 1fr', gap: 14 }}>
            ...
            <div>
              ...
              <input className="input tnum" type="number" step={unitStep} value={stock} onChange={(e) => setStock(e.target.value)} style={{ fontSize: 18, fontWeight: 800, textAlign: 'center' }} placeholder={isWeight ? '10' : '180'} />
              <div style={{ fontSize: 11.5, color: 'var(--ink-3)', fontWeight: 600, marginTop: 5, textAlign: 'center' }}>
                Unidad base: <strong>{unit}</strong>
              </div>
            </div>
            {/* Peso por unidad — solo si la unidad base NO es peso */}
            {!isWeight && (
              <div>
                ...
                <input className="input tnum" type="number" step="0.001" value={kgPerUnit || ''} onChange={(e) => setKgPerUnit(e.target.value)} style={{ fontSize: 15, fontWeight: 700, textAlign: 'center' }} placeholder="0.070" />
                <div style={{ fontSize: 11.5, color: 'var(--ink-3)', fontWeight: 600, marginTop: 5, textAlign: 'center' }}>{+kgPerUnit > 0 ? `Total: ${totalKg} kg` : `Ej: 1 ${unit} = 0.07 kg`}</div>
              </div>
            )}
            {/* Costo por unidad */}
            <div>
              ...
              <div className="input-pre" style={{ background: 'var(--surface)' }}>
                ...
                <input className="tnum" type="number" value={cost} onChange={(e) => setCost(e.target.value)} style={{ padding: '10px 13px 10px 4px', fontSize: 15, fontWeight: 700, textAlign: 'right' }} />
              </div>
              <div style={{ fontSize: 11.5, color: 'var(--ink-3)', fontWeight: 600, marginTop: 5, textAlign: 'center' }}>Costo total: {fmtCLP(totalCost)}</div>
            </div>
          </div>
          {/* Tira de resumen */}
        ...
        <div>
                  ...
                  {fmts.map((f) => {
                    ...
                    return (
                      <tr key={f.id}>
                        ...
                        <td>
                          <button className="btn btn-ghost btn-icon" style={{ width: 28, height: 28 }} onClick={() => deleteFormat(f.id)}>
          ...
          {adding ? (
            <div style={{ marginTop: 12, padding: '16px', background: 'var(--surface-3)', borderRadius: 13, border: '1px dashed var(--primary)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 10, marginBottom: 10 }}>
                ...
                <label className="field">
                  <span style={{ fontSize: 11.5, fontWeight: 700, color: 'var(--ink-2)' }}>Cantidad ({unitAbbrev}) que descuenta</span>
              ...
              {newFmt.qty &&
                newFmt.price &&
                (() => {
                  const c = variantCost(newFmt.qty)
                  const m = variantMargin(newFmt.price, newFmt.qty)
                  const mp = variantMarginPct(newFmt.price, newFmt.qty)
                  const cards: { l: string; v: string; danger?: boolean }[] = [
                    { l: 'Costo calculado', v: fmtCLP(c) },
                    { l: 'Ganancia', v: fmtCLP(m) },
                    { l: 'Margen', v: mp + '%', danger: mp < 25 },
                    { l: 'Disponibles', v: maxUnits(newFmt.qty) + ' u.' },
                  ]
                  return (
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                <button className="btn btn-primary" disabled={!newFmt.name || !newFmt.qty || !newFmt.price} onClick={addVariant}>
                  <Icon name="check" size={15} />
                  Agregar variante
                </button>
                <button
          ...
          <div style={{ marginTop: 14, padding: '10px 14px', background: 'var(--primary-tint)', borderRadius: 10, fontSize: 13, fontWeight: 600, color: 'var(--primary-700)', display: 'flex', gap: 9, alignItems: 'center' }}>
            ...
            Al registrar una venta, el sistema descuenta automáticamente las unidades del stock madre.
          </div>
        </div>
      </div>
    </Modal>
...
/* ── FormatPicker en línea: para la pantalla de Ventas ───── */
export function FormatPicker({ product, onPick, onCancel }: { product: ProductWithKg; onPick: (product: ProductWithKg, fmt: Format, qty: number) => void; onCancel: () => void }) {
  const { getFormats, canSellFormat, maxUnitsForFormat } = useFormats()
  const fmts = getFormats(product.id)
  const [qty, setQty] = useState(1)
  const [selFmt, setSelFmt] = useState<string | null>(fmts[0]?.id || null)

  const check = selFmt ? canSellFormat(product, selFmt, qty) : { ok: false, needed: 0, available: 0 }
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--primary)', borderRadius: 16, padding: '18px', boxShadow: 'var(--sh-2)', marginBottom: 10 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12.5, color: 'var(--ink-3)', fontWeight: 600, marginTop: 2 }}>
            Stock: <strong>{product.stock} u.</strong>
            {(product.kgPerUnit || 0) > 0 && ` · ${+(product.stock * product.kgPerUnit!).toFixed(2)} kg`}
          </div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(130px,1fr))', gap: 8, marginBottom: 14 }}>
        {fmts.map((f) => {
          const chk = canSellFormat(product, f.id, 1)
          const active = selFmt === f.id
          const kg = (product.kgPerUnit || 0) > 0 ? +(product.kgPerUnit! * f.qty).toFixed(3) : null
          return (
            <button
              key={f.id}
              onClick={() => {
              ...
              disabled={!chk.ok}
              style={{ padding: '11px 12px', textAlign: 'left', borderRadius: 12, border: `2px solid ${active ? 'var(--primary)' : 'var(--line)'}`, background: active ? 'var(--primary-tint)' : chk.ok ? 'var(--surface)' : 'var(--surface-3)', opacity: chk.ok ? 1 : 0.5, cursor: chk.ok ? 'pointer' : 'not-allowed', transition: '.13s', fontFamily: 'inherit' }}
            >
              ...
              <div style={{ fontSize: 11, color: chk.ok ? 'var(--primary-700)' : 'var(--danger)', fontWeight: 700, marginTop: 2 }}>{chk.ok ? `Hasta ${Math.floor(product.stock / f.qty)} u.` : 'Sin stock'}</div>
            </button>
          )
        })}
      </div>
      ...
      {selFmt && !check.ok && (
        <div style={{ padding: '10px 13px', background: 'var(--danger-tint)', borderRadius: 10, fontSize: 13, fontWeight: 600, color: 'var(--danger)', marginBottom: 12, display: 'flex', gap: 8, alignItems: 'center' }}>
          <Icon name="alert" size={14} />
          Sin stock suficiente. Quedan {check.available} unidades.
        </div>
      )}
        ...
        <div style={{ padding: '12px 14px', background: 'var(--surface-3)', borderRadius: 12, marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <button className="btn btn-ghost btn-icon" style={{ width: 30, height: 30 }} onClick={() => setQty((q) => Math.max(1, q - 1))}>
                <Icon name="minus" size={14} />
              </button>
              <span className="tnum" style={{ width: 28, textAlign: 'center', fontWeight: 800, fontSize: 16 }}>{qty}</span>
              <button className="btn btn-ghost btn-icon" style={{ width: 30, height: 30 }} onClick={() => setQty((q) => Math.min(maxQty, q + 1))} disabled={qty >= maxQty}>
      ...
      <button
        className="btn btn-primary"
        style={{ width: '100%' }}
        disabled={!selFmt || !check.ok}
        onClick={() => {
          if (fmt && check.ok) onPick(product, fmt, qty)
        }}
      >
        <Icon name="plus" size={16} />
        Agregar al carrito · {selFmt && fmt ? fmtCLP(fmt.price * qty) : '—'}
      </button>
    </div>
  )
}

/* ── FormatBreakdown: vista de inventario ─────── */
export function FormatBreakdown({ product }: { product: ProductWithKg }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <button onClick={() => setOpen((v) => !v)} className="btn btn-ghost" style={{ padding: '6px 11px', fontSize: 12.5, gap: 6 }}>
        ...
        {open ? 'Ocultar' : 'Ver'} variantes ({fmts.length})
      ...
      {open && (
        <div style={{ marginTop: 8, padding: '12px 14px', background: 'var(--surface-3)', borderRadius: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--ink-3)', marginBottom: 3 }}>
            Stock base: {product.stock} u.{(product.kgPerUnit || 0) > 0 ? ` · ${+(product.stock * product.kgPerUnit!).toFixed(2)} kg` : ''}
          </div>
          {fmts.map((f) => {
            const max = maxUnitsForFormat(product, f.id)
            const ok = max > 0
            const kg = (product.kgPerUnit || 0) > 0 ? +(product.kgPerUnit! * f.qty).toFixed(3) : null
            const rowStyle: CSSProperties = { display: 'flex', alignItems: 'center', gap: 10, padding: '7px 10px', background: ok ? 'var(--surface)' : 'var(--danger-tint)', borderRadius: 9, border: `1px solid ${ok ? 'var(--line)' : 'var(--danger-tint)'}` }
            return (
              <div key={f.id} style={rowStyle}>
                <span style={{ flex: 1, fontWeight: 700, fontSize: 13 }}>{f.name}</span>
                <span style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 600 }}>
                  {f.qty} u.{kg != null ? ` / ${kg} kg` : ''}
                </span>
                <span className="chip" style={{ background: ok ? 'var(--ok-tint)' : 'var(--danger-tint)', color: ok ? 'var(--primary-700)' : 'var(--danger)', fontSize: 12, fontWeight: 800, minWidth: 54, textAlign: 'center' }}>{ok ? `Hasta ${max}` : 'Sin stock'}</span>
              </div>
            )
          })}
        </div>
