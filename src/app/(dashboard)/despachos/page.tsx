...
// ---------- Despachos: pedidos, rutas, repartidores (portado de screen-despachos.jsx) ----------
import { useEffect, useRef, useState } from 'react'
import { useStore } from '@/lib/store'
import { fmtCLP } from '@/lib/format'
import { Icon } from '@/components/icon'
import { PageHeader, Metric, Modal, EmptyState, Mini } from '@/components/ui'
import type { Sale, SaleItem, Cliente } from '@/types'

type Estado = 'pendiente' | 'en_ruta' | 'entregado' | 'no_entregado'
...
interface Pedido {
  ...
  total: number
  method: string
  _onEntregado?: (id: string) => void
}

/* ── Seed data ───────────────────────────────────────── */
function seedDespachos(sales: Sale[], clientes: Cliente[]): Pedido[] {
  let s = 55123
  const rnd = () => {
  ...
  const estados: Estado[] = ['pendiente', 'en_ruta', 'entregado', 'entregado', 'entregado', 'no_entregado']
  return sales
    .filter((_, i) => i < 12)
    .map((sale, i) => {
      ...
      return {
        ...
        direccion: dir,
        ciudad: cl.ciudad,
        nota: rnd() < 0.3 ? 'Dejar en conserjería' : '',
        repartidor: estado === 'pendiente' ? 'Ninguno asignado' : rep,
        estado,
        items: sale.items,
        total: sale.total,
        method: sale.method,
...
const ESTADO_CFG: Record<Estado, { label: string; bg: string; fg: string; icon: string }> = {
...
function PedidoModal({ pedido, onClose, onUpdate }: { pedido: Pedido; onClose: () => void; onUpdate: (id: string, patch: Partial<Pedido>) => void }) {
  ...
  return (
    <Modal
      ...
      sub={pedido.cliente + ' · ' + pedido.ciudad}
      onClose={onClose}
      width={560}
      footer={
        <>
          ...
          <button
            ...
            onClick={() => {
              onUpdate(pedido.id, { estado, nota: obs, repartidor: rep })
              onClose()
            }}
          >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 8 }}>
            {(Object.entries(ESTADO_CFG) as [Estado, (typeof ESTADO_CFG)[Estado]][]).map(([k, c]) => (
              <button
                key={k}
                onClick={() => setEstado(k)}
                style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '9px 12px', borderRadius: 11, border: `2px solid ${estado === k ? c.fg : 'var(--line)'}`, background: estado === k ? c.bg : 'var(--surface)', cursor: 'pointer', fontFamily: 'inherit', fontWeight: 700, fontSize: 13.5, color: estado === k ? c.fg : 'var(--ink-2)', transition: '.12s' }}
              >
          ...
          {orden.map((p, i) => {
            ...
            return (
              <div
                ...
                onDrop={handleDrop}
                className="card"
                style={{ border: '1px solid var(--line)', cursor: 'grab', transition: 'box-shadow .15s', boxShadow: dragOverIdx.current === i ? '0 0 0 2px var(--primary), var(--sh-2)' : 'var(--sh-1)' }}
              >
                {/* Header row */}
                <div style={{ padding: '15px 18px', display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 3, padding: '6px 4px', color: 'var(--ink-3)', cursor: 'grab', flexShrink: 0 }}>
                    {[0, 1, 2].map((k) => (
                      <div key={k} style={{ display: 'flex', gap: 3 }}>
                        {[0, 1].map((j) => (
                          <div key={j} style={{ width: 4, height: 4, borderRadius: 2, background: 'var(--line-2)' }}></div>
                        ))}
                      </div>
                    ))}
                  </div>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div className="tnum" style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--ink-3)', marginTop: 3 }}>
                      {p.items.length} productos · {fmtCLP(p.total)} · {p.method}
                    </div>
                  </div>
                ...
                {mapOpen && (
                  <div style={{ padding: '0 18px 14px' }}>
                    <div style={{ borderRadius: 12, overflow: 'hidden', border: '1px solid var(--line)', position: 'relative' }}>
                      <iframe src={mapsEmbedUrl} width="100%" height="220" style={{ border: 'none', display: 'block' }} loading="lazy" referrerPolicy="no-referrer-when-downgrade" title={`Mapa ${p.direccion}`} />
                      <a
                        href={mapsOpenUrl}
                        target="_blank"
                ...
                <div style={{ padding: '10px 18px 14px', borderTop: '1px solid var(--line)', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  ...
                  <a href={waUrl} target="_blank" rel="noopener noreferrer" style={{ textDecoration: 'none', flex: 1 }}>
                    <button className="btn btn-soft" style={{ width: '100%', fontSize: 13 }}>
                      <Icon name="phone" size={15} />
                      WhatsApp
                    </button>
                  </a>
...
export default function DespachosPage() {
  const { sales, clientes, toast } = useStore()
  const [pedidos, setPedidos] = useState<Pedido[]>(() => seedDespachos(sales, clientes))
  ...
  const updatePedido = (id: string, patch: Partial<Pedido>) => setPedidos((ps) => ps.map((p) => (p.id === id ? { ...p, ...patch } : p)))
  ...
  const pedidosParaRuta = pedidos
    .filter((p) => p.estado === 'pendiente' || p.estado === 'en_ruta')
    .map((p) => ({
      ...p,
      _onEntregado: (id: string) => {
        updatePedido(id, { estado: 'entregado' })
        toast('¡Entregado! ' + p.cliente)
      },
    }))
    ...
    <div className="fade-in">
      ...
      {counts.pendiente > 0 && (
        <button className="card card-pad" onClick={() => setVistaRuta(true)} style={{ display: 'flex', alignItems: 'center', gap: 13, marginBottom: 18, cursor: 'pointer', textAlign: 'left', width: '100%', border: '1px solid var(--primary-tint2)' }}>
          <span style={{ width: 40, height: 40, borderRadius: 12, background: 'var(--primary)', color: '#fff', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
              ...
              <tbody>
                {list.map((p) => (
                  <tr key={p.id} style={{ cursor: 'pointer' }} onClick={() => setSelected(p)} onMouseEnter={(e) => (e.currentTarget.style.background = 'var(--surface-3)')} onMouseLeave={(e) => (e.currentTarget.style.background = '')}>
                    <td className="tnum" style={{ fontWeight: 700, color: 'var(--ink-2)' }}>
                      # {p.boleta}
                    </td>
                    <td>
                    ...
                    <td style={{ color: 'var(--ink-2)', fontWeight: 600, maxWidth: 200 }}>
                      <div style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.direccion}</div>
                    </td>
        ...
        <div className="grid" style={{ gridTemplateColumns: '1fr 1fr 1fr', marginTop: 18 }}>
          <div className="card card-pad">
            ...
            {cities
              .filter((c) => c !== 'Todas')
              .map((c) => {
                const n = pedidos.filter((p) => p.ciudad === c).length
                return (
          ...
          <div className="card card-pad">
            ...
            {reps
              .filter((r) => r !== 'Todos')
              .map((r) => {
                const n = pedidos.filter((p) => p.repartidor === r).length
                return (
          ...
          <div className="card card-pad">
            ...
            {([['entregado', 'Entregados'], ['no_entregado', 'Fallidos'], ['en_ruta', 'En ruta']] as [string, string][]).map(([k, l]) => {
              ...
              return (
                <div key={k} style={{ marginBottom: 9 }}>
                  ...
                  <div style={{ height: 6, background: 'var(--bg-2)', borderRadius: 4 }}>
                    <div style={{ height: '100%', width: pct + '%', background: k === 'entregado' ? 'var(--primary)' : k === 'no_entregado' ? 'var(--danger)' : 'var(--info)', borderRadius: 4 }}></div>
                  </div>
                </div>
              )
            })}
