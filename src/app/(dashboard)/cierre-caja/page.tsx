...
const TODAY_LABEL = new Date().toLocaleDateString('es-CL', { weekday: 'long', day: 'numeric', month: 'long' })
...
export default function CierreCajaPage() {
  ...
  const totalGanancia = resumen?.totalGanancia ?? 0
  const totalEfect   = resumen?.efectivo       ?? 0
  const totalTarjeta = resumen?.tarjeta        ?? 0
  const totalTrans   = resumen?.transferencia  ?? 0
  const totalCredito = resumen?.creditoPendiente ?? 0
  const abonosHoy    = resumen?.abonosHoy      ?? 0
  const nBoletas     = resumen?.nBoletas        ?? 0
  const nItems       = resumen?.nItems          ?? 0
  const lowStock     = resumen?.productosStockBajo ?? 0
  ...
  const handleCerrar = async () => {
    ...
    try {
      ...
      await guardarCierre({
        efectivoFinal: efectivoContado !== '' ? +efectivoContado : totalEfect,
        diferencia:    difTotal,
        observaciones: notas,
      })
      setConfirmed(true)
  ...
  const DifChip = ({ d }: { d: number | null }) =>
    d === null ? null : d === 0 ? (
      <span className="chip chip-ok" style={{ fontSize: 12 }}>Exacto</span>
    ) : d > 0 ? (
      <span className="chip chip-ok" style={{ fontSize: 12 }}>+{fmtCLP(d)}</span>
    ) : (
      <span className="chip chip-danger" style={{ fontSize: 12 }}>{fmtCLP(d)}</span>
    )
  ...
  return (
    <div className="fade-in" style={{ maxWidth: 900 }}>
      <PageHeader title="Cierre de caja" sub={`Resumen del día · ${TODAY_LABEL}`}>
        ...
        <div className="card" style={{ textAlign: 'center', padding: '48px 32px' }}>
          ...
          <div className="grid" style={{ gridTemplateColumns: 'repeat(3,1fr)', gap: 12, maxWidth: 480, margin: '0 auto 24px' }}>
            <div style={{ padding: '12px', background: 'var(--surface-3)', borderRadius: 12, textAlign: 'center' }}>
              <div className="tnum" style={{ fontWeight: 800, fontSize: 18, color: 'var(--primary-700)' }}>{fmtCLP(totalVentas)}</div>
              <div style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 700 }}>Ventas</div>
            </div>
            <div style={{ padding: '12px', background: 'var(--surface-3)', borderRadius: 12, textAlign: 'center' }}>
              <div className="tnum" style={{ fontWeight: 800, fontSize: 18, color: 'var(--terra-700)' }}>{fmtCLP(totalGanancia)}</div>
              <div style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 700 }}>Ganancia</div>
            </div>
            <div style={{ padding: '12px', background: 'var(--surface-3)', borderRadius: 12, textAlign: 'center' }}>
              <div className="tnum" style={{ fontWeight: 800, fontSize: 18 }}>{nBoletas}</div>
              <div style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 700 }}>Boletas</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
            <button className="btn btn-ghost" onClick={() => { setConfirmed(false); setEfectivoContado(''); setTarjetaContado(''); setTransContado('') }}>
              Nuevo cierre
            </button>
        ...
        <div className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 18, alignItems: 'start' }}>
              ...
              <div className="card-head">
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div className="card-sub">
                    {loadingResumen ? 'Cargando…' : `${nBoletas} boletas · ${nItems} artículos vendidos`}
                  </div>
                </div>
              </div>
                ...
                <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {cierres.slice(0, 5).map((c) => (
                    <div key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 0', borderBottom: '1px solid var(--line)' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontWeight: 700, fontSize: 13.5 }}>{fmtCLP(c.total_ventas)}</div>
                        <div style={{ fontSize: 12, color: 'var(--ink-3)', fontWeight: 600 }}>
                          ...
                          {c.diferencia !== 0 && (
                            <span style={{ marginLeft: 6, color: c.diferencia > 0 ? 'var(--primary-700)' : 'var(--danger)' }}>
                              dif: {c.diferencia > 0 ? '+' : ''}{fmtCLP(c.diferencia)}
                            </span>
                          )}
                        </div>
                      </div>
                      <span className="tnum" style={{ fontWeight: 700, fontSize: 13, color: 'var(--ink-2)' }}>
                        ef: {fmtCLP(c.efectivo_final)}
                      </span>
                    </div>
                  ))}
                </div>
          ...
          <div className="card">
            ...
            <div style={{ padding: '16px 22px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                ...
                <div key={i}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: 13, fontWeight: 700 }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <span className="tnum muted" style={{ fontSize: 12 }}>Sistema: {fmtCLP(f.sys)}</span>
                      <DifChip d={f.dif} />
                    </span>
                  </div>
              ...
              <button className="btn btn-primary btn-lg" style={{ width: '100%', marginTop: 4 }} onClick={handleCerrar} disabled={saving || loadingResumen}>
