...
function ProveedorModal({ initial, onSave, onClose }: { initial?: Proveedor; onSave: (p: Proveedor) => void; onClose: () => void }) {
  ...
  const toggleCat = (c: string) => set('categorias', f.categorias.includes(c) ? f.categorias.filter((x) => x !== c) : [...f.categorias, c])
  return (
    <Modal
      title={initial ? 'Editar proveedor' : 'Agregar proveedor'}
      sub="Registra contacto, categorías y condiciones de pago"
      onClose={onClose}
      width={520}
      footer={
        <>
          ...
          <button
            ...
            onClick={() => {
              onSave({ ...f, id: initial?.id || 'pv' + Date.now(), pedidos: initial?.pedidos || 0, ultimoPedido: initial?.ultimoPedido || new Date() })
              onClose()
            }}
          >
      ...
      <div style={{ display: 'grid', gap: 14 }}>
        <div className="grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          <Field label="Nombre del proveedor">
            <input className="input" value={f.nombre} onChange={(e) => set('nombre', e.target.value)} placeholder="Ej: Huevos del Valle" autoFocus />
          </Field>
          <Field label="Contacto">
            <input className="input" value={f.contacto} onChange={(e) => set('contacto', e.target.value)} placeholder="Nombre de la persona" />
          </Field>
        </div>
        <div className="grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          <Field label="Teléfono / WhatsApp">
            <input className="input" value={f.telefono} onChange={(e) => set('telefono', e.target.value)} placeholder="+56912345678" />
          </Field>
          <Field label="Correo">
        ...
        <div className="grid" style={{ gridTemplateColumns: '1fr 1fr' }}>
          <Field label="Condición de pago">
            <select className="select" value={f.condicion} onChange={(e) => set('condicion', e.target.value)}>
          ...
          <Field label="Estado">
            <select className="select" value={f.activo ? 'activo' : 'inactivo'} onChange={(e) => set('activo', e.target.value === 'activo')}>
        ...
        <Field label="Notas internas">
          <input className="input" value={f.notas} onChange={(e) => set('notas', e.target.value)} placeholder="Ej: pedido mínimo, días de entrega, etc." />
        </Field>
      </div>
    </Modal>
  )
...
export default function ProveedoresPage() {
  ...
  const prodsByProv = (pv: Proveedor) => products.filter((p) => pv.categorias.includes(p.cat))
  ...
  return (
    <div className="fade-in">
      ...
      <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(170px,1fr))', marginBottom: 18 }}>
        ...
        <Metric icon="clock" label="Último pedido" value={proveedores.filter((p) => p.ultimoPedido).sort((a, b) => b.ultimoPedido.getTime() - a.ultimoPedido.getTime())[0]?.nombre.split(' ')[0] || '—'} tone="primary" />
      </div>

      <div style={{ display: 'flex', gap: 7, marginBottom: 16, overflowX: 'auto', paddingBottom: 2 }}>
        {cats.map((c) => (
          <button key={c} onClick={() => setCat(c)} className="chip" style={{ border: '1px solid var(--line)', cursor: 'pointer', whiteSpace: 'nowrap', background: cat === c ? 'var(--primary)' : 'var(--surface)', color: cat === c ? '#fff' : 'var(--ink-2)', padding: '7px 14px', fontSize: 13 }}>
            {c}
          </button>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 16 }}>
        {list.map((pv) => {
          const diasDesde = pv.ultimoPedido ? Math.round((TODAY.getTime() - pv.ultimoPedido.getTime()) / 86400000) : null
          ...
          return (
            <div key={pv.id} className="card" style={{ padding: '18px 20px', opacity: pv.activo ? 1 : 0.6 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                {pv.categorias.map((c) => (
                  <span key={c} className="chip chip-neutral" style={{ fontSize: 11.5, padding: '3px 9px' }}>
                    {c}
                  </span>
                ))}
              </div>
              ...
              {bajos.length > 0 && (
                <div style={{ padding: '7px 10px', background: 'var(--warn-tint)', borderRadius: 9, fontSize: 12.5, fontWeight: 700, color: 'oklch(0.50 0.10 70)', marginBottom: 10 }}>
                  <Icon name="alert" size={13} style={{ verticalAlign: '-2px', marginRight: 5 }} />
                  {bajos.length} producto{bajos.length !== 1 ? 's' : ''} por reponer
                </div>
              )}
              <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
                {pv.telefono && (
                  <a href={`https://wa.me/${pv.telefono.replace(/\D/g, '')}?text=${encodeURIComponent('Hola ' + pv.contacto.split(' ')[0] + ', me gustaría hacer un pedido.')}`} target="_blank" rel="noopener noreferrer">
                    <button className="btn btn-ghost" style={{ fontSize: 12.5, padding: '6px 11px' }}>
                ...
                {pv.correo && (
                  <a href={`mailto:${pv.correo}?subject=Pedido ${new Date().toLocaleDateString('es-CL')}`}>
                    <button className="btn btn-ghost" style={{ fontSize: 12.5, padding: '6px 11px' }}>
                ...
                <button
                  ...
                  style={{ fontSize: 12.5, padding: '6px 11px', marginLeft: 'auto' }}
                  onClick={() => {
                    updateProv(pv.id, { pedidos: pv.pedidos + 1, ultimoPedido: new Date() })
        ...
        {list.length === 0 && (
          <div style={{ gridColumn: '1/-1' }}>
            <EmptyState
              ...
              text="Agrega tus proveedores para tener sus datos a mano al reponer stock."
              action={
                <button className="btn btn-primary" onClick={() => setForm(true)}>
