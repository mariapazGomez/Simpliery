...
function mensajeError(e: unknown): string {
  ...
  if (m.includes('invalid login credentials') || m.includes('incorrect')) return 'Usuario o contraseña incorrectos.'
  if (m.includes('already registered') || m.includes('already exists')) return 'Ese correo ya tiene una cuenta. Inicia sesión.'
  if (m.includes('password should be at least')) return 'La contraseña debe tener al menos 4 caracteres.'
...
export default function LoginPage() {
  ...
  const [tab, setTab]       = useState<LoginTab>('dueno')
  ...
  const submitDueno = async (e: React.FormEvent) => {
    e.preventDefault()
  ...
  const submitEmpleado = async (e: React.FormEvent) => {
    e.preventDefault()
    setErr(null); setMsg(null); setLoading(true)
    const supabase = createClient()
    try {
      ...
      const syntheticEmail = `${usernameNorm}@${codigoNorm.toLowerCase()}.controllocal`
      ...
      // Obtener rol para saber a dónde redirigir
      ...
      if (user) {
        const { data: perfil } = await supabase
          ...
          .eq('id', user.id)
          .maybeSingle()
        const rol = (perfil as { rol?: string } | null)?.rol ?? 'vendedor'
        router.replace(DESTINO_POR_ROL[rol] ?? '/ventas')
      } else {
  ...
  const switchTab = (t: LoginTab) => { setTab(t); setErr(null); setMsg(null) }
    ...
    <div style={{ minHeight: '100dvh', display: 'grid', placeItems: 'center', padding: 20, background: 'var(--surface-2, #f6f5f1)' }}>
      <div className="card" style={{ width: '100%', maxWidth: 400 }}>
        <div className="card-pad" style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
          ...
          {/* Tabs */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, padding: 4, background: 'var(--surface-2)', borderRadius: 12 }}>
            {(['dueno', 'empleado'] as LoginTab[]).map((t) => (
              <button
                key={t}
                type="button"
                onClick={() => switchTab(t)}
                style={{
                  ...
                  boxShadow:  tab === t ? 'var(--sh1, 0 1px 3px rgba(0,0,0,.1))' : 'none',
                }}
              >
                {t === 'dueno' ? 'Dueño / Admin' : 'Empleado'}
              </button>
            ))}
          </div>
          ...
          {tab === 'dueno' && (
            <>
              <p style={{ fontSize: 13.5, color: 'var(--ink-3)', fontWeight: 600, margin: 0, textAlign: 'center' }}>
                {mode === 'login' ? 'Entra a tu negocio' : 'Crea la cuenta de tu negocio'}
              </p>

              <form onSubmit={submitDueno} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {mode === 'signup' && (
                  <div>
          ...
          {tab === 'empleado' && (
            <>
              <p style={{ fontSize: 13.5, color: 'var(--ink-3)', fontWeight: 600, margin: 0, textAlign: 'center' }}>
                Pídele al dueño el código de la tienda y tus credenciales
              </p>

              <form onSubmit={submitEmpleado} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div>
                  <label style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--ink-2)' }}>Código de la tienda</label>
                  <input
                    className="input"
                    style={{ marginTop: 5, textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 700 }}
                    value={codigo}
                ...
                <div>
                  <label style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--ink-2)' }}>Tu usuario</label>
                  <input
                    className="input"
                    style={{ marginTop: 5 }}
                    value={username}
                ...
                <div>
                  <label style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--ink-2)' }}>PIN</label>
                  <input
                    className="input"
                    style={{ marginTop: 5 }}
                    type="password"
