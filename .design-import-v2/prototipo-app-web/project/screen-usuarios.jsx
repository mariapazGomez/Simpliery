/* ---------- Usuarios: gestión de equipo y roles ---------- */
...
const SEED_USERS = [
  { id:'u1', nombre:'Marta Soto',     email:'marta@emporiodonамarta.cl', rol:'admin',    activo:true,  avatar:'M', ultimoAcceso: new Date(2026,5,8,14,32), accionesDia:23 },
...
function PermisosMatrix({ rol, custom, onChange }){
  ...
  return (
    <div style={{display:'grid', gridTemplateColumns:'1fr 1fr', gap:8}}>
      {Object.entries(PERMISOS).map(([k,label])=>{
        ...
        return (
          <label key={k} style={{display:'flex', alignItems:'center', gap:10, padding:'8px 11px',
...
function InviteModal({ onClose, onSave }){
  const [f, setF] = useState({ nombre:'', email:'', rol:'vendedor', custom:[] });
  const set = (k,v)=> setF(s=>({...s,[k]:v}));
  const togglePerm = (k, on)=> setF(s=>({ ...s, custom: on?[...s.custom,k]:s.custom.filter(x=>x!==k) }));
          ...
          <div style={{display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8}}>
            {Object.values(ROLES).map(r=>(
              <button key={r.id} onClick={()=>set('rol',r.id)} className="btn" style={{
                ...
                <span style={{fontSize:11, opacity:.8, lineHeight:1.2}}>{r.desc.slice(0,28)}</span>
              </button>
            ))}
          </div>
        </Field>
...
function EditRolModal({ usuario, onClose, onSave }){
  const [rol, setRol] = useState(usuario.rol);
  const [custom, setCustom] = useState(usuario.custom||[]);
  const togglePerm = (k,on)=> setCustom(c=> on?[...c,k]:c.filter(x=>x!==k));
  return (
...
function Usuarios(){
  ...
  const saveUser = (id, patch)=> setUsers(us=>us.map(u=>u.id===id?{...u,...patch}:u));
  const toggleActive = id=> setUsers(us=>us.map(u=>u.id===id?{...u,activo:!u.activo}:u));
                ...
                <tr key={u.id} style={{opacity: u.activo?1:0.55}}>
                  <td>
                    <div style={{display:'flex', alignItems:'center', gap:12}}>
                      <div style={{width:38,height:38,borderRadius:11,background: u.activo?'var(--primary)':'var(--line-2)',
                        color:'#fff', display:'grid', placeItems:'center', fontWeight:800, fontSize:15, flexShrink:0}}>
                        {u.avatar}
                      </div>
                      <div>
