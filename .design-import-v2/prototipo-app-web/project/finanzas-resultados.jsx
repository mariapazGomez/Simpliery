/* ---------- Finanzas: Estado de Resultados + Cuentas por cobrar + Cuentas por pagar ---------- */
...
function FinResultados(){
  ...
  const periods = [['hoy','Hoy'],['semana','Semana'],['mes','Este mes'],['anterior','Mes anterior']];
  ...
  const bycat = GASTO_CATS.reduce((a,c)=>{ a[c]=gastosMes.filter(g=>g.cat===c).reduce((s,g)=>s+g.monto,0); return a; },{});
  ...
  const pct = (v)=> ventasNetas ? (v/ventasNetas*100).toFixed(1)+'%' : '—';
  const var_prev = (v)=> {
  ...
  const rows_op = Object.entries(bycat).filter(([,v])=>v>0).map(([cat,v])=>({ label:cat, v:-v, indent:1, icon: GASTO_ICONS[cat], color: GASTO_COLORS[cat] }));
  const rows_end = [
  ...
  function TRow({ row }){
    ...
    const hl = { primary:'var(--primary-tint)', ok:'var(--ok-tint)', warn:'var(--warn-tint)', danger:'var(--danger-tint)' };
    const fgHL = { primary:'var(--primary-700)', ok:'var(--primary-700)', warn:'oklch(0.50 0.10 70)', danger:'var(--danger)' };
  ...
  return (
    <div className="fade-in">
      ...
      <div className="grid" style={{gridTemplateColumns:'repeat(auto-fit,minmax(160px,1fr))', marginBottom:18}}>
        ...
        ].map((c,i)=><FinCard key={i} icon={c.icon} label={c.label} value={fmtCLP(c.v)} tone={c.tone} sub={c.sub}/>)}
      </div>
...
window.FinResultados = FinResultados;
