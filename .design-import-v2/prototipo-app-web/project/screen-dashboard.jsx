...
function Dashboard({ go }){
  ...
  const alerts = [
    ...
    proxClientes>0 && { icon:'clientes', tone:'primary', text:`${proxClientes} clientes están próximos a recomprar esta semana`, cta:'Ver segmentos', to:'segmentos' },
    (()=>{ const lowM=m.cats.find(c=>c.marginPct<settings.minMargin); return lowM && { icon:'percent', tone:'warn', text:`Margen de ${lowM.cat} bajo el ${settings.minMargin}% — revisa precios`, cta:'Ver productos', to:'productos' }; })(),
    m.topCat && { icon:'zap', tone:'primary', text:`${m.topCat.cat} es tu categoría estrella del mes`, cta:'Ver reportes', to:'reportes' },
  ].filter(Boolean);

  return (
    <div className="fade-in">
      <PageHeader title={`Hola, ${settings.business.split(' ')[0]==='Emporio'?'Marta':settings.business} 👋`} sub={`Resumen de hoy · ${TODAY.toLocaleDateString('es-CL',{weekday:'long', day:'numeric', month:'long'})}`}>
        ...
        }}><Icon name="phone" size={16}/>Compartir por WA</button>
        <button className="btn btn-ghost"><Icon name="download" size={16}/>Exportar</button>
        <button className="btn btn-primary" onClick={()=>go('ventas')}><Icon name="plus" size={16}/>Registrar venta</button>
      </PageHeader>
          ...
          {alerts.map((a,i)=>{
            ...
            return (
              <button key={i} onClick={()=>go(a.to)} className="card card-pad" style={{display:'flex', alignItems:'center', gap:13, textAlign:'left', cursor:'pointer', border:'1px solid var(--line)'}}>
                <span style={{width:36,height:36,borderRadius:10,background:bg,color:fg,display:'grid',placeItems:'center',flexShrink:0}}><Icon name={a.icon} size={18}/></span>
                <span style={{flex:1, fontSize:13.5, fontWeight:600, lineHeight:1.35}}>{a.text}</span>
                <span style={{color:'var(--ink-3)'}}><Icon name="chevR" size={16}/></span>
              </button>
            );
          })}
        </div>
      ...
      <div className="grid" style={{gridTemplateColumns:'1.5fr 1fr', alignItems:'start'}}>
        ...
        <div className="card">
          <div className="card-head">
            <div style={{flex:1}}><div className="card-title">Ventas de la semana</div><div className="card-sub">Últimos 7 días</div></div>
            <div className="tnum" style={{textAlign:'right'}}>
              <div style={{fontWeight:800, fontSize:18}}>{fmtCLP(trend.reduce((a,b)=>a+b,0))}</div>
              <div style={{fontSize:12, color:'var(--primary-700)', fontWeight:700, display:'flex', alignItems:'center', gap:4, justifyContent:'flex-end'}}><Icon name="trendUp" size={13}/>+14% vs. semana anterior</div>
            </div>
          </div>
          <div className="card-pad">
            ...
            <div style={{display:'flex', justifyContent:'space-between', marginTop:8, fontSize:11.5, color:'var(--ink-3)', fontWeight:700}}>
              {['Sáb','Dom','Lun','Mar','Mié','Jue','Hoy'].map(d=><span key={d}>{d}</span>)}
            </div>
          </div>
        </div>
        ...
        {/* Payment methods */}
        <div className="card">
          <div className="card-head"><div style={{flex:1}}><div className="card-title">Métodos de pago</div><div className="card-sub">Ventas de hoy</div></div></div>
          <div className="card-pad" style={{display:'flex', alignItems:'center', gap:20}}>
            <Donut size={140} thickness={22}
              data={payRows.map(([k,v])=>({value:v, color:payColors[k]||'var(--ink-3)'}))}
              centerValue={fmtCLP(payTotal).replace('$','$')} centerLabel="Total hoy"/>
      ...
      <div className="grid" style={{gridTemplateColumns:'1fr 1fr', marginTop:18, alignItems:'start'}}>
        {/* Sales by category */}
        <div className="card">
          <div className="card-head"><div style={{flex:1}}><div className="card-title">Ventas por categoría</div><div className="card-sub">Ingresos del mes</div></div></div>
          <div className="card-pad">
            <BarList rows={m.cats.slice(0,6).map(c=>({label:c.cat, value:c.revenue, color:c.color}))}/>
          </div>
        </div>

        {/* Ingresos vs costos vs ganancia by category */}
        <div className="card">
          <div className="card-head"><div style={{flex:1}}><div className="card-title">Ingresos, costos y ganancia</div><div className="card-sub">Top 4 categorías del mes</div></div></div>
          <div className="card-pad">
            <ColumnChart height={170}
              groups={m.cats.slice(0,4).map(c=>({label:c.cat.split(' ')[0], revenue:c.revenue, cost:c.cost, profit:c.profit}))}
              series={[ {key:'revenue', label:'Ingresos', color:'var(--primary)'}, {key:'cost', label:'Costos', color:'var(--line-2)'}, {key:'profit', label:'Ganancia', color:'var(--terra)'} ]}/>
          </div>
        </div>
      </div>
      ...
      <div className="card" style={{marginTop:18}}>
        <div className="card-head"><div style={{flex:1}}><div className="card-title">Productos más vendidos</div><div className="card-sub">Por unidades este mes</div></div>
          <button className="btn btn-ghost" onClick={()=>go('reportes')}>Ver todos<Icon name="chevR" size={15}/></button></div>
        <div style={{overflowX:'auto'}}>
          <table className="tbl">
            <thead><tr><th style={{width:40}}>#</th><th>Producto</th><th>Categoría</th><th className="num">Vendidos</th><th className="num">Ingresos</th><th className="num">Margen</th></tr></thead>
            <tbody>
              {m.topProducts.map((p,i)=>(
                <tr key={p.id}>
                  <td><span className="tnum" style={{fontWeight:800, color: i<3?'var(--terra-700)':'var(--ink-3)'}}>{i+1}</span></td>
