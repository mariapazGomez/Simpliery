/* ---------- Comprobante de venta — diseño imprimible ---------- */
...
function ComprobanteModal({ sale, negocio, onClose }){
  ...
  const handlePrint = ()=>{
    // Clone receipt into print-root
    const root = document.getElementById('print-root') || (()=>{ const d=document.createElement('div'); d.id='print-root'; document.body.appendChild(d); return d; })();
    root.innerHTML = printRef.current.innerHTML;
    window.print();
    setTimeout(()=>{ root.innerHTML=''; }, 1500);
  };

  const lineStyle = { display:'flex', justifyContent:'space-between', fontSize:13, padding:'3px 0' };
  const divStyle  = { borderTop:'1px dashed #ccc', margin:'8px 0' };
    ...
    <div className="overlay" onMouseDown={onClose}>
      <div className="modal scale-in" style={{maxWidth:440}} onMouseDown={e=>e.stopPropagation()}>
        ...
        <div className="card-head no-print" style={{position:'sticky',top:0,background:'var(--surface)',borderRadius:'24px 24px 0 0',zIndex:2}}>
          <span style={{width:34,height:34,borderRadius:10,background:'var(--primary-tint)',color:'var(--primary-700)',display:'grid',placeItems:'center'}}><Icon name="receipt" size={18}/></span>
          <div style={{flex:1}}><div className="card-title">Comprobante listo</div><div className="card-sub">N° {sale.boleta}</div></div>
          <button className="btn btn-ghost btn-icon no-print" onClick={onClose}><Icon name="x" size={18}/></button>
        </div>
        ...
        <div style={{padding:'20px 22px'}}>
          <div ref={printRef} style={{fontFamily:"'Courier New', Courier, monospace", fontSize:13, color:'#1a1a1a', background:'#fff', padding:'20px', maxWidth:340, margin:'0 auto'}}>
            ...
            <div style={{...lineStyle, fontSize:12}}>
              <span>Fecha:</span><span>{sale.date.toLocaleDateString('es-CL',{day:'2-digit',month:'2-digit',year:'numeric'})}</span>
            </div>
            <div style={{...lineStyle, fontSize:12}}>
              <span>Hora:</span><span>{sale.date.toLocaleTimeString('es-CL',{hour:'2-digit',minute:'2-digit'})}</span>
            </div>
            {hasCliente && (
              <div style={{...lineStyle, fontSize:12}}>
                <span>Cliente:</span><span style={{fontWeight:700}}>{sale.cliente.nombre}</span>
              </div>
            )}
            <div style={divStyle}/>
            ...
            {/* Totals */}
            <div style={lineStyle}>
              <span>Subtotal</span><span className="tnum">{fmtCLP(sale.total)}</span>
            </div>
            {(sale.descuento?.amount||0)>0 && (
              <div style={{...lineStyle, color:'var(--primary-700)', fontWeight:700}}>
                <span>Descuento</span><span className="tnum">-{fmtCLP(sale.descuento.amount)}</span>
              </div>
            )}
            <div style={{...lineStyle, fontWeight:900, fontSize:16, marginTop:4}}>
              <span>TOTAL</span><span className="tnum">{fmtCLP(totalFinal)}</span>
            </div>
            <div style={{...lineStyle, fontSize:12, color:'#555'}}>
              <span>Método de pago</span><span>{sale.method}</span>
            </div>
            {sale.method==='Crédito' && (
              <div style={{...lineStyle, fontSize:12, color:'#c00', fontWeight:700}}>
                <span>Saldo pendiente</span><span>{fmtCLP(sale.balance||sale.total)}</span>
              </div>
            )}
            <div style={divStyle}/>
            ...
            <div style={{textAlign:'center', fontSize:11, color:'#888', lineHeight:1.6}}>
              ...
              <div style={{marginTop:6, fontSize:10.5, color:'#aaa', fontFamily:'sans-serif'}}>
                Este documento es un comprobante de venta.<br/>No reemplaza boleta ni factura tributaria.
              </div>
            </div>
          </div>
          ...
          <div className="no-print" style={{display:'flex',gap:8,marginTop:16,justifyContent:'center',flexWrap:'wrap'}}>
            <button className="btn btn-ghost" style={{fontSize:13,padding:'9px 16px'}} onClick={handlePrint}>
              <Icon name="download" size={15}/>Imprimir / PDF
            </button>
            {(sale.cliente?.numero||'').replace(/\D/g,'').length>=8 && (
              <a href={`https://wa.me/56${(sale.cliente.numero||'').replace(/\D/g,'')}?text=${waText}`} target="_blank" rel="noopener noreferrer">
                <button className="btn btn-ghost" style={{fontSize:13,padding:'9px 16px'}}>
                  <Icon name="phone" size={15}/>Enviar WA
                </button>
              </a>
            )}
          ...
          <div className="no-print" style={{textAlign:'center',fontSize:11.5,color:'var(--ink-3)',marginTop:10,fontWeight:600}}>
            "Imprimir / PDF" abre el diálogo de impresión del navegador.<br/>Selecciona "Guardar como PDF" para guardar.
          </div>
        </div>
      </div>
    </div>,
