/* ---------- Finanzas: Deudas y Créditos + Balance Simplificado ---------- */
...
const CREDITO_ICONS = { 'Préstamo bancario':'building','Línea de crédito':'wallet','Leasing':'truck','Deuda proveedor':'box','Tarjeta de crédito':'card','Crédito FOGAPE':'shield','Otro':'coins' };
const CREDITO_COLORS = { 'Préstamo bancario':'var(--info)','Línea de crédito':'var(--primary)','Leasing':'var(--terra)','Deuda proveedor':'oklch(0.62 0.10 130)','Tarjeta de crédito':'var(--danger)','Crédito FOGAPE':'oklch(0.60 0.10 180)','Otro':'var(--ink-3)' };

function CreditoModal({ initial, onSave, onClose }){
  const [f,setF]=useState(initial||{ acreedor:'', tipo:CREDITO_TIPOS[0], montoOriginal:'', saldo:'', tasaAnual:'', cuotaMensual:'', proximaCuota: new Date(TODAY.getFullYear(),TODAY.getMonth()+1,5), estado:'vigente', notas:'', moneda:'CLP' });
  ...
  return (
    <Modal title={initial?'Editar crédito':'Agregar deuda o crédito'} sub="Registra préstamos, líneas, leasing o deudas pendientes" onClose={onClose} width={560}
      footer={<><button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
        <button className="btn btn-primary" disabled={!valid} onClick={()=>{ onSave({...f, montoOriginal:+f.montoOriginal, saldo:+f.saldo||+f.montoOriginal, tasaAnual:+f.tasaAnual||0, cuotaMensual:+f.cuotaMensual||0, id:initial?.id||'cr'+Date.now(), pagos:initial?.pagos||[] }); onClose(); }}><Icon name="check" size={16}/>Guardar</button></>}>
...
function PagoModal({ credito, onPago, onClose }){
  const [monto,setMonto]=useState(credito.cuotaMensual||'');
  ...
  return (
    <Modal title="Registrar pago" sub={credito.acreedor} onClose={onClose} width={440}
      footer={<><button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
...
function FinPasivos(){
  ...
  const estadoChip = (e)=>({ vigente:<span className="chip chip-neutral" style={{fontSize:12}}>Vigente</span>, al_dia:<span className="chip chip-ok" style={{fontSize:12}}>Al día</span>, atrasado:<span className="chip chip-danger" style={{fontSize:12}}>Atrasado</span>, pagado:<span className="chip chip-ok" style={{fontSize:12}}>Pagado</span> }[e]);
  ...
  return (
    <div className="fade-in">
            ...
            <table className="tbl" style={{minWidth:700}}>
              ...
              <tbody>
                {vigentes.map(c=>{
                  ...
                  const diasProx = Math.round((c.proximaCuota-TODAY)/86400000);
          ...
          <div style={{padding:'16px 22px'}}>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:14}}>
              {[0,1,2].map(mes=>{
                ...
                const cuotas = vigentes.filter(c=>{ const p=new Date(c.proximaCuota); p.setMonth(p.getMonth()+mes); return true; }).reduce((a,c)=>a+c.cuotaMensual,0);
                const gastos = m.totalGastosMes;
                const libre = Math.round(ingrEst*(m.gananciaMes/m.ingresosMes||0.3) - gastos - cuotas);
                return (
      ...
      {showHist && (
        <Modal title="Historial de pagos" sub={showHist.acreedor} onClose={()=>setShowHist(null)} width={480}
          footer={<button className="btn btn-ghost" onClick={()=>setShowHist(null)}>Cerrar</button>}>
          {(showHist.pagos||[]).length===0 ? <EmptyState icon="history" title="Sin pagos registrados" text="Los pagos que registres aparecerán aquí."/> : (
              ...
              {showHist.pagos.map((p,i)=>(
                <div key={i} style={{display:'flex',alignItems:'center',gap:12,padding:'12px 0',borderBottom:'1px solid var(--line)'}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <div style={{fontSize:12,color:'var(--ink-3)',fontWeight:600}}>{p.nota} · Int. {fmtCLP(p.interes)} · Amort. {fmtCLP(p.amortizacion)}</div>
                  </div>
                  <div style={{textAlign:'right',fontSize:12.5,fontWeight:700,color:'var(--ink-3)'}}>
                    ...
                    <div style={{fontSize:11.5,fontWeight:600,marginTop:2}}>Saldo: {fmtCLP(p.saldoDespues)}</div>
                  </div>
                </div>
              ))}
            </div>
...
function FinBalance(){
  ...
  const healthTone = ratioApalancamiento>70?'danger':ratioApalancamiento>45?'warn':'ok';
  ...
  function BalRow({ label, value, indent, bold, highlight, sub, tone }){
  ...
  function GroupHdr({ label, total, tone }){
    const c = { ok:'var(--primary-700)', danger:'var(--danger)', warn:'oklch(0.50 0.10 70)', info:'var(--info)' }[tone]||'var(--ink-3)';
    return (
...
Object.assign(window, { FinPasivos, FinBalance });
