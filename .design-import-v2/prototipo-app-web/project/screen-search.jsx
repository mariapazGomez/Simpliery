...
function GlobalSearch({ onClose, go }){
  ...
  const results = useMemo(()=>{
    ...
    const r=[];
    // Products
    products.filter(p=>p.name.toLowerCase().includes(ql)||p.cat.toLowerCase().includes(ql)).slice(0,5).forEach(p=>
      r.push({type:'Producto',icon:'productos',label:p.name,sub:`${p.cat} · ${fmtCLP(p.price)} · Stock ${p.stock}`,action:()=>{ go('productos'); onClose(); }}));
    // Clients
    clientes.filter(c=>c.nombre.toLowerCase().includes(ql)||(c.telefono||'').includes(q)||(c.correo||'').toLowerCase().includes(ql)).slice(0,4).forEach(c=>
      r.push({type:'Cliente',icon:'clientes',label:c.nombre,sub:`${c.ciudad||''} · ${c.telefono||''}`,action:()=>{ go('clientes'); onClose(); }}));
    // Sales
    sales.filter(s=>String(s.boleta).includes(q)||s.items.some(i=>i.name.toLowerCase().includes(ql))).slice(0,3).forEach(s=>
      r.push({type:'Venta',icon:'receipt',label:`Boleta #${s.boleta}`,sub:`${fmtCLP(s.total)} · ${s.method} · ${s.date.toLocaleDateString('es-CL')}`,action:()=>{ go('ventas'); onClose(); }}));
    return r;
  },[q,products,clientes,sales]);

  const typeColor={ Producto:'var(--primary-tint)', Cliente:'var(--info-tint)', Venta:'var(--terra-tint)' };
  const typeFg={ Producto:'var(--primary-700)', Cliente:'var(--info)', Venta:'var(--terra-700)' };
  const shortcuts=[['Inicio','dashboard'],['Ventas','ventas'],['Inventario','inventario'],['Clientes','clientes'],['Finanzas','finanzas'],['Reportes','reportes']];
    ...
    <div style={{position:'fixed',inset:0,background:'rgba(43,38,32,0.45)',backdropFilter:'blur(4px)',zIndex:200,display:'flex',alignItems:'flex-start',justifyContent:'center',paddingTop:80}} onMouseDown={onClose}>
      <div className="scale-in" style={{width:'100%',maxWidth:580,background:'var(--surface)',borderRadius:18,boxShadow:'var(--sh-pop)',overflow:'hidden'}} onMouseDown={e=>e.stopPropagation()}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12,padding:'14px 18px',borderBottom:'1px solid var(--line)'}}>
          ...
          {q && <button style={{background:'none',border:'none',cursor:'pointer',color:'var(--ink-3)'}} onClick={()=>setQ('')}><Icon name="x" size={16}/></button>}
        ...
        {q.length>=2 ? (
          results.length>0 ? (
            <div style={{maxHeight:400,overflowY:'auto'}}>
              {results.map((r,i)=>(
                <button key={i} onClick={r.action} style={{width:'100%',display:'flex',alignItems:'center',gap:13,padding:'12px 18px',background:'none',border:'none',borderBottom:'1px solid var(--line)',cursor:'pointer',textAlign:'left',transition:'.12s',fontFamily:'inherit'}}
                  onMouseEnter={e=>e.currentTarget.style.background='var(--surface-3)'} onMouseLeave={e=>e.currentTarget.style.background='none'}>
                  <span style={{width:34,height:34,borderRadius:9,background:typeColor[r.type],color:typeFg[r.type],display:'grid',placeItems:'center',flexShrink:0}}><Icon name={r.icon} size={16}/></span>
          ...
          <div style={{padding:'16px 18px'}}>
            ...
            <div style={{display:'flex',gap:7,flexWrap:'wrap'}}>
              {shortcuts.map(([l,r])=>(
                <button key={r} onClick={()=>{ go(r); onClose(); }} className="chip chip-neutral" style={{cursor:'pointer',fontSize:13.5,padding:'7px 13px',fontFamily:'inherit'}}>{l}</button>
              ))}
            </div>
