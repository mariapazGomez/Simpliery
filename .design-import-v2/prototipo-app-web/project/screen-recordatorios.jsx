/* ---------- Recordatorios: centro de tareas del negocio ---------- */
...
const STATUSES = { pendiente:'Pendiente', en_proceso:'En proceso', completado:'Completado', vencido:'Vencido', pospuesto:'Pospuesto' };

function seedReminders(products, clientes){
  const r = [];
  let id = 1;
  const add = (type, title, desc, due, priority='media', status='pendiente', link=null)=>
...
function ReminderCard({ r, onComplete, onPostpone, onDelete, go }){
  ...
  const daysLeft = Math.ceil((r.due - new Date())/(1000*60*60*24));

  return (
    <div className="card" style={{ opacity:isDone?0.6:1, border:`1px solid ${isOverdue?'var(--danger)':t.color}20`,
      transition:'opacity .2s' }}>
      <div style={{display:'flex', gap:14, padding:'15px 18px', alignItems:'flex-start'}}>
        <span style={{width:38,height:38,borderRadius:11,background:isDone?'var(--surface-3)':t.tint,
          color:isDone?'var(--ink-3)':t.color,display:'grid',placeItems:'center',flexShrink:0}}>
        ...
        <div style={{flex:1, minWidth:0}}>
          <div style={{display:'flex', alignItems:'flex-start', gap:10, flexWrap:'wrap', marginBottom:4}}>
            ...
            <span className="chip" style={{fontSize:11.5, padding:'2px 8px', background:
              r.priority==='alta'?'var(--danger-tint)': r.priority==='media'?'var(--warn-tint)':'var(--surface-3)',
          ...
          <div style={{display:'flex', gap:8, flexWrap:'wrap', alignItems:'center'}}>
            ...
            {!isDone && (
              <span className="chip" style={{background:isOverdue?'var(--danger-tint)':daysLeft<=2?'var(--warn-tint)':'var(--surface-3)',
...
function NewReminderModal({ onClose, onSave }){
  ...
  const [f, setF] = useState({ type:'tarea', title:'', desc:'', due:'', priority:'media' });
  ...
  return (
    <Modal title="Nuevo recordatorio" sub="Crea una tarea o recordatorio para tu negocio" onClose={onClose} width={520}
      footer={<><button className="btn btn-ghost" onClick={onClose}>Cancelar</button>
        <button className="btn btn-primary" disabled={!valid} onClick={()=>{ onSave({...f,id:'rem'+Date.now(),due:new Date(f.due),status:'pendiente',createdAt:new Date()}); onClose(); }}><Icon name="plus" size={15}/>Agregar</button></>}>
...
function Recordatorios({ go }){
  const { products, clientes } = useStore();
  const m = useMetrics();
  const [reminders, setReminders] = useState(()=> seedReminders(products, clientes));
  ...
  const debtReminders = useMemo(()=>{
    ...
    return [{ id:'auto_deuda', type:'cliente', title:`Cobrar deuda pendiente (${fmtCLP(m.totalDeuda)})`,
  ...
  const filtered = useMemo(()=>{
    ...
    return list.sort((a,b)=>{
      const p={alta:0,media:1,baja:2}; return (p[a.priority]||1)-(p[b.priority]||1) || a.due-b.due;
    });
  },[allReminders, filter, typeFilter]);

  const complete = id=> setReminders(rs=>rs.map(r=>r.id===id?{...r,status:'completado'}:r));
  const postpone = id=> setReminders(rs=>rs.map(r=>r.id===id?{...r,due:new Date(r.due.getTime()+3*24*60*60*1000),status:'pospuesto'}:r));
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{display:'flex', gap:10, marginBottom:14, flexWrap:'wrap', alignItems:'center'}}>
        <div className="seg">
          {[['pendientes','Pendientes'],['vencidos','Vencidos'],['hoy','Hoy'],['todos','Todos'],['completados','Completados']].map(([k,l])=>(
